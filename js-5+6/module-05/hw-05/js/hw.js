 ...
 */
const getUsersByEyeColor = (arr, eyeColor) => {
  return arr.filter(obj => obj.eyeColor === eyeColor)
};
...
const getUsersByGender = (arr, gender) => {
  const objGenders = arr.filter(obj => obj.gender === gender);
  return objGenders.map(objGender => objGender.name);

  //return arr.filter(obj => obj.gender === gender).map(objGender => objGender.name);
};
...
const getInactiveUsers = arr => {
  return arr.filter(obj => !obj.isActive);
};
...
const getUserByEmail = (arr, email) => {
  return arr.find(obj => obj.email === email);
};
...
const getUsersWithAge = (arr, min, max) => {
  return arr.filter(obj => min < obj.age && obj.age < max);
};
...
const getTotalBalance = arr => {
  ...
  return arr.reduce((acc, obj) => acc + obj.balance, 0);
};
...
const getUsersByFriend = (arr, name) => {
  ...
  const userWisFrends = arr.filter(obj => obj.friends.find(el => el === name));
  
  return userWisFrends.map(userWisFrend => userWisFrend.name);
};
